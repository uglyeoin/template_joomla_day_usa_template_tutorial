...
if($params->get('icons') > 0 && $params->get('icons') < 3) {
	...
	if(!empty($faicon)) {
		if($item->params->get('menu_text', 1)) {
			$linktype = '<em class="dj-icon '.$faicon.'" aria-hidden="true"></em><span class="image-title">'. $item->title . $subtitle .'</span>';
		} else {
			$linktype = '&nbsp;<em class="dj-icon '.$faicon.'" aria-hidden="true" title="'.htmlspecialchars($item->title).'"></em>&nbsp;';
